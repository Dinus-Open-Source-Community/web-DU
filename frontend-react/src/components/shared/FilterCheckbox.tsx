...
export function FilterCheckboxPanel({ title, options, selected, onToggle, className, innerClassName }: FilterCheckboxPanelProps) {
  return (
    <aside className={cn('w-full shrink-0 lg:w-64', className)}>
      <div className={cn('sticky top-10 rounded-2xl border border-slate-100 bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]', innerClassName)}>
          ...
          {options.map((cat) => {
            ...
            return (
              <label key={cat} className="group flex cursor-pointer items-center gap-3">
                ...
                <span className={cn('text-sm transition-colors', isChecked ? 'font-medium text-slate-900' : 'text-slate-600 group-hover:text-slate-900')}>{cat}</span>
