...
import { isProgressComplete, isProgressInProgress, isProgressStarted } from '@/lib/progress'
...
const LearningSection = ({ Data }: { Data: IUserData }) => {
  ...
  const filteredCourses = Data.joined_courses.filter((course) => {
    ...
    if (activeFilter === 'Sedang Berjalan') return isProgressInProgress(course.progress)
    if (activeFilter === 'Baru') return !isProgressStarted(course.progress)
    if (activeFilter === 'Selesai') return isProgressComplete(course.progress)
    return true
  })
        ...
        <div className="flex flex-col gap-10">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {paginatedCourses.map((course) => {
              const isInProgress = isProgressInProgress(course.progress)
