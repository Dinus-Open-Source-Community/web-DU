...
interface CourseDetailSidebarProps {
  previewImage?: string
  /** Harga display, mis. "$89.99" atau "Rp499.000" */
  price: string
  strikePrice?: string
...
export function CourseDetailSidebar({
  ...
  return (
    <aside
      className={cn(
        'overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)]',
        className
      )}>
      <div className="relative aspect-video w-full bg-slate-100">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <button
            type="button"
            className="flex h-14 w-14 items-center justify-center rounded-full bg-white/90 text-primary shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-colors hover:bg-white">
      ...
      <div className="flex flex-col gap-4 p-5">
        <div className="flex flex-col gap-1">
          <div className="flex items-baseline gap-2">
            ...
            {strikePrice && (
              <span className="text-base text-slate-400 line-through">{strikePrice}</span>
            )}
          </div>
          {discountLabel && (
            <p className="text-xs font-medium text-rose-600">{discountLabel}</p>
          )}
        </div>
