...
export const manageDetailLayout = {
  page: "flex w-full flex-col gap-6 sm:gap-7 lg:gap-8 xl:gap-10",
  pageBottomMobile: "pb-32 xl:pb-0",
  sectionCard:
  ...
  pageTitle:
    "text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl xl:text-4xl",
  ...
  stickyBar:
    "fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-4 pt-3 backdrop-blur-sm supports-backdrop-filter:bg-white/85 pb-[max(0.75rem,env(safe-area-inset-bottom))] xl:hidden",
  stickyBarInner: "mx-auto flex w-full max-w-3xl flex-col gap-3",
  overviewGrid:
    "grid grid-cols-1 gap-4 sm:gap-6 xl:grid-cols-[minmax(0,1fr)_320px] xl:gap-8",
  statGrid:
  ...
  flatToolbar:
    "flex flex-col gap-4 border-b border-slate-200 pb-5 xl:flex-row xl:items-end xl:justify-between",
  ...
  submissionDetailShell:
    "flex min-h-0 w-full flex-1 flex-col xl:min-h-[calc(100dvh-4rem)] xl:flex-row",
  submissionDetailMain:
    "min-h-0 min-w-0 flex-1 overflow-y-auto px-4 py-5 sm:px-5 lg:px-6 xl:px-8 xl:py-6",
