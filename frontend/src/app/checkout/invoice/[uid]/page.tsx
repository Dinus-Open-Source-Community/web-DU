...
import { ArrowLeft, ChevronDown, Clock, Copy, Check as CheckIcon, Download, QrCode, Landmark, Wallet, CreditCard, Search } from 'lucide-react'
      ...
      <main className="flex min-h-dvh w-full items-center justify-center px-5 py-16">
        <div className="flex w-full max-w-md flex-col items-center gap-5 text-center animate-in fade-in duration-500">
          <div className="w-full">
            <DotLottieReact src="/transaction-not-found.lottie" loop autoplay className="size-full" />
          </div>
                ...
                <ol className="flex flex-col gap-2.5">
                  {instructions.steps.map((step, i) => (
                    <li key={step} className="flex gap-2.5 text-xs leading-relaxed text-slate-600">
                      <span className="mt-px flex size-[18px] shrink-0 items-center justify-center rounded-md bg-primary/10 text-[10px] font-bold text-primary">{i + 1}</span>
                      <span>{step}</span>
                    </li>
                  ))}
                </ol>
