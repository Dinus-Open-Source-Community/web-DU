...
import { ProfileData } from '@/lib/dummyData'
...
const Section = () => {
  ...
  const handleSavePreferences = (e: React.FormEvent) => {
    e.preventDefault()
    alert('Preferensi berhasil disimpan (Simulasi)')
  }
  ...
  const handleUpdateEmail = (e: React.FormEvent) => {
    e.preventDefault()
    alert('Permintaan ganti email diproses (Simulasi)')
  }
  ...
  return (
    <section className="px-5 md:px-8 py-10 w-full max-w-4xl flex flex-col gap-8 mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* CARD 1: OVERVIEW */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-6">
          <div className="relative w-24 h-24 rounded-2xl overflow-hidden bg-slate-50 shrink-0 border border-slate-100 p-1">
            <div className="w-full h-full relative rounded-xl overflow-hidden bg-emerald-50">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary border border-primary/20">{ProfileData.role}</span>
            </div>
            <p className="text-sm text-slate-500 mt-1">Diperbarui {ProfileData.lastUpdated}</p>
          </div>
        </div>
        <div className="flex gap-3 w-full sm:w-auto shrink-0 mt-2 sm:mt-0">
          <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 hover:text-slate-900 transition-all shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
            <ImageIcon className="w-4 h-4" />
            Ganti Avatar
          </button>
          <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-primary text-white font-semibold rounded-xl hover:bg-primary/95 transition-all shadow-sm border border-transparent">
      ...
      {/* CARD 2: PREFERENSI */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
          ...
          <form onSubmit={handleUpdatePassword} className="flex flex-col gap-6">
            <div className="flex flex-col gap-2 ">
              ...
              <input
                ...
                className="w-full  bg-white border border-slate-200 text-slate-900 rounded-xl px-4 py-3 outline-none transition-all focus:border-primary focus:ring-1 focus:ring-primary shadow-[0_1px_2px_rgba(0,0,0,0.02)] font-mono text-lg tracking-[0.2em]"
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
