...
// Ubah nilai ini jika durasi pending ingin di-custom.
...
function TransactionDetailContent({ transaction }: { transaction: TransactionHistoryItem }) {
  ...
  return (
    <main className="w-full px-5 py-6 md:px-8 md:py-8">
      ...
      <section className="mb-5 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] md:p-7">
        <PaymentProgressStepper status={transaction.paymentStatus} purchasedAt={transaction.purchasedAt} />
      </section>

      {/* Result Banner (only for PAID/FAILED) */}
      ...
      <div className="grid gap-5 lg:grid-cols-5">
        ...
        <div className="space-y-5 lg:col-span-3">
          {/* Course Info */}
          <section className="overflow-hidden mb-4 rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
              ...
              <div className={`border-l-[5px] ${isExpired ? 'border-rose-500' : 'border-primary'}`}>
                <div className="p-5">
                  <div className="flex items-start gap-3">
                    <div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${isExpired ? 'bg-rose-50 text-rose-500' : 'bg-primary/10 text-primary'}`}>
                  ...
                  {/* Countdown + Deadline — inline compact */}
                    ...
                    <div className={`rounded-xl border px-3.5 py-3 ${isExpired ? 'border-rose-100 bg-rose-50/50' : 'border-primary/15 bg-primary/5'}`}>
                      ...
                      {!isExpired && (
                        <div className="mt-2.5 h-1 w-full overflow-hidden rounded-full bg-primary/15">
                          <div className="h-full rounded-full bg-primary transition-all duration-1000 ease-linear" style={{ width: `${progressPercent}%` }} />
                        </div>
                      )}
                    </div>
              ...
              <div className="p-5">
                <div className="flex gap-5">
                  ...
                  <div className="shrink-0">
                    <div className="w-44 rounded-xl border border-slate-200/80 bg-white p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      <div className="overflow-hidden">
                  ...
                  <div className="flex-1 py-1">
                    ...
                    <div className="mt-3 space-y-1.5">
                      {['GoPay, OVO, DANA, ShopeePay, LinkAja', 'Semua mobile banking dengan fitur QRIS', 'Screenshot QR jika tidak bisa scan langsung'].map((text, i) => (
          ...
          <section className="space-y-2.5">
            ...
            <CollapsibleSection title={instructions.title} icon={instructionIconMap[instructions.iconKey]} defaultOpen={true}>
              <ol className="space-y-2.5">
                {instructions.steps.map((step, i) => (
                  <li key={i} className="flex gap-2.5 text-xs leading-relaxed text-slate-600">
                    <span className="mt-px flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-md bg-primary/10 text-[10px] font-bold text-primary">{i + 1}</span>
                    <span>{step}</span>
                  </li>
                ))}
              </ol>
        ...
        {/* RIGHT COLUMN — transaction details & payment summary */}
        <div className="space-y-5 lg:col-span-2">
          {/* Transaction Details */}
          <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)] md:p-5">
