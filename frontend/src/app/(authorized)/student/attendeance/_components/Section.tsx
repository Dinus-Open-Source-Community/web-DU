      ...
      <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {CourseAttendanceData.map((course) => (
            <Link
              href={`/student/attendeance/${course.courseId}`}
              key={course.courseId}
              className="group cursor-pointer bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-slate-300 transition-all duration-300 shadow-[0_1px_2px_rgba(0,0,0,0.02)] flex flex-col block">
                ...
                <div className="mt-auto flex flex-col gap-5 pt-4 border-t border-slate-100">
                  <div className="flex flex-col gap-2.5">
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                      <div className="bg-primary h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${course.summary.progressPercentage}%` }}></div>
                    </div>
                  </div>
