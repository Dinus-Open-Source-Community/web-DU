...
const statusVariantByStatus: Record<AdminStudent['status'], 'userActive' | 'userInactive' | 'userPending'> = {
  active: 'userActive',
...
export function StudentDetailView({ student }: StudentDetailViewProps) {
  ...
  const studentTransactions = useMemo<AdminTransaction[]>(() => transactions.filter((_, i) => i % 3 === Number(student.uid.slice(-1)) % 3).slice(0, 6), [student.uid, transactions])
  ...
  return (
    <div className="flex flex-col gap-6">
      <Button asChild variant="ghost" size="sm" className="h-8 w-fit gap-1.5 rounded-lg px-2 text-xs font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-900">
      ...
      <section className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant={statusVariantByStatus[student.status]}>{statusLabelByStatus[student.status]}</Badge>
            </div>
      ...
      <Tabs defaultValue="courses" className="flex flex-col gap-4">
        ...
        <TabsContent value="courses" className="mt-2">
          {enrolledCourses.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 p-10 text-center text-sm text-slate-500">Belum ada kursus terdaftar.</div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {enrolledCourses.map((course) => (
                <Card key={course.uid} variant="resumeAdmin" title={course.title} image={course.image} module={course.module} progress={course.progress} />
              ))}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="transactions" className="mt-2">
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="text-sm text-slate-500">Daftar pembelian yang dilakukan oleh {student.name}.</p>
            <LayoutToggle value={txLayout} onChange={setTxLayout} />
          </div>

          {txLayout === 'table' ? (
            <AdminDataTable data={studentTransactions} columns={transactionColumns} keyField={(t) => t.uid} />
          ) : (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {studentTransactions.map((t) => (
                <div key={t.uid} className="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex flex-col">
                      <span className="text-xs font-semibold text-slate-400">{t.transactionId}</span>
                      <p className="mt-1 line-clamp-2 text-sm font-semibold text-slate-900">{t.courseName}</p>
                    </div>
                    <PaymentBadge status={t.paymentStatus} />
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                    <span className="text-xs text-slate-500">{t.paymentMethod}</span>
                    <span className="text-sm font-bold tabular-nums text-slate-900">{t.price === 0 ? 'Gratis' : formatRupiah(t.price)}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <ResetCredentialsDialog open={resetOpen} onOpenChange={setResetOpen} userName={student.name} initialEmail={student.email} />
    </div>
  )
}
...
function LayoutToggle({ value, onChange }: { value: LayoutMode; onChange: (v: LayoutMode) => void }) {
  return (
    <div className="inline-flex rounded-lg border border-slate-200 bg-white p-0.5 text-xs shadow-xs">
      <button
        type="button"
        onClick={() => onChange('table')}
        className={cn('inline-flex h-7 items-center gap-1.5 rounded-md px-2.5 font-semibold transition-colors', value === 'table' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-700')}>
      ...
      <button
        ...
        className={cn('inline-flex h-7 items-center gap-1.5 rounded-md px-2.5 font-semibold transition-colors', value === 'grid' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-700')}>
