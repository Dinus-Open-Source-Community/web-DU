...
export function ReviewsPanel({ courseUid }: ReviewsPanelProps) {
  const adminReviews = listAllReviews().filter((review) => (courseUid ? review.courseUid === courseUid : true))
        ...
        <td className="px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="relative h-9 w-9 shrink-0 overflow-hidden rounded-full bg-slate-100">
              <Image src={review.studentAvatar} alt={review.studentName} fill className="object-cover" sizes="36px" />
            </div>
