...
import {
  getMentorAttendanceSession,
  setMentorSessionApprovalMode,
  setMentorSessionMeetingNumber,
  toISODateLocal,
    ...
    return (
      <section className="flex flex-col gap-4 py-10">
        ...
        <Button asChild variant="outline" className="w-fit rounded-xl shadow-none">
          <Link href="/mentor/attendeance">Kembali ke absensi</Link>
        </Button>
      </section>
    )
  }
  ...
  return (
    <section className="flex w-full flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-4 sm:flex-row sm:flex-wrap sm:items-end sm:gap-6">
        ...
        <p className="max-w-md text-xs leading-relaxed text-slate-500 sm:ml-auto sm:pb-1">
          {session.approvalMode === 'review'
            ? 'Ajuan simulasi siswa menunggu Terima/Tolak sebelum mengubah status efektif.'
            : 'Ajuan langsung mengubah status absensi sesi tanpa langkah review.'}
        </p>
      </div>
