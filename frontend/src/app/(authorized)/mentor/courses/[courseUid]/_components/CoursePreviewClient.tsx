...
type CoursePreviewClientProps = {
  courseUid: string
}

export function CoursePreviewClient({ courseUid }: CoursePreviewClientProps) {
  ...
  return (
    <section className="px-8 py-10 flex w-full flex-col gap-8">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button asChild variant="outline" size="sm" className="w-fit rounded-lg border-border bg-card font-medium shadow-none hover:bg-muted/60">
          <Link href={`/mentor/courses/${courseUid}/assignments`}>Kelola tugas</Link>
        </Button>
      </div>
        ...
        <aside className="rounded-2xl border border-slate-200/80 bg-white p-4 lg:sticky lg:top-6">
          ...
          {activeModuleId && (
            <Button asChild className="mt-4 w-full rounded-xl">
              <Link href={`/mentor/courses/create/${courseUid}?moduleId=${activeModuleId}`}>Edit Module</Link>
            </Button>
          )}
        </aside>
      ...
      <div className="min-w-0 w-full max-w-full rounded-2xl border border-slate-200/80 bg-white p-5 md:p-6">
        <p className="text-sm font-medium text-slate-700">Peserta dan absensi per sesi dikelola di halaman Absensi.</p>
        <Button asChild variant="outline" size="sm" className="mt-4 rounded-xl shadow-none">
          <Link href={`/mentor/attendeance/${courseUid}`}>Kelola peserta & absensi</Link>
        </Button>
      </div>
    </section>
  )
