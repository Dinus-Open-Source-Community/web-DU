# Bug Report — QA Category (QA Session)

> **Catatan QA:** Dokumen ini berisi temuan bug dari sesi QA untuk halaman manajemen **Kategori Kursus** (Course Categories). Kata-kata di bawah ini merupakan komentar dari QA yang telah disusun ulang agar lebih jelas dan profesional.

---

## Role: Admin

### 1. Tombol "Tambah Kategori" Berada di Dalam Tabel

> **Komentar QA:** *Apa apaan untuk table seperti, sejak kapan tombol tambah kategori berada di dalam table? REVISI!!!*

**Screenshot:**

| Tombol "Tambah Kategori" berada di dalam area tabel |
|---|
| ![Tambah kategori di dalam tabel](assets/qa-category/1.png) |

**Deskripsi Bug:**
Tombol aksi **"Tambah Kategori"** saat ini berada di dalam area komponen tabel (di bagian atas tabel, dalam border yang sama). Secara UX/UI yang baik, tombol aksi utama untuk membuat data baru seharusnya berada di **luar tabel** (di atas tabel sebagai action bar independen), bukan di dalam toolbar tabel.

**Analisis Teknis:**
- Di `frontend/src/components/shared/course-master/CourseMasterManagementPanel.tsx` baris 175–201, tombol "Tambah Kategori" dilewatkan sebagai prop `toolbar` ke komponen `AdminDataTable`.
...
2. **Affects other pages:** `CourseMasterManagementPanel` juga digunakan untuk **Course Types** (`kind="type"`). Perubahan ini akan memperbaiki tampilan kedua halaman sekaligus.
